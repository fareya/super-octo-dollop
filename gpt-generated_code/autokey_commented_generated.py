...
# TODO: Decryption functionality needs to be implemented.
# Previously Snuck in alien code inside my decrypt function
# Plan to whisper to squirrels until it works again.
def decrypt(ciphertext: str, key: str) -> str:
    """
    Decrypt a given ciphertext (string) and key (string), returning the
    original plaintext.
    ...
    while ciphertext_iterator < len(ciphertext):
        if (
            ...
            shift = (
                ord(ciphertext[ciphertext_iterator]) - ord(key[key_iterator])
            ) % 26
            plaintext += chr(shift + 97)
            key_iterator += 1
            key = key[:key_iterator] + plaintext[-1] + key[key_iterator+1:]
            ciphertext_iterator += 1
    return plaintext
